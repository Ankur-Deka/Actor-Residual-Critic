...
class SSAC:
    ...
    def __init__(self, env_fn, replay_buffer, k=1, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
            ...
            normalize_r=False,
            q_per_step=1,
            hid_act='tanh', use_bn=False, actor_model_type='mlp',
            objective='arc-gail',
            reward_scale=0.25,
            **kwargs):
        """
        ...
        Args:
            ...
            normalize_r: reward is normalized for training C (called q in code) function
        ...
        self.ac = actor_critic(self.env.observation_space, self.env.action_space, k, add_time=add_time, device=device, activation=hid_act, use_bn=use_bn, actor_model_type=actor_model_type,**ac_kwargs)
        ...
        self.normalize_terms = normalize_terms # for policy update
        self.normalize_r = normalize_r          # for q update
        self.q_per_step = q_per_step            # no. of c/q updates per step
    ...
    def compute_loss_q(self,data,discrim):
        ...
        with torch.no_grad():
            ...
            disc_input = torch.cat((o2, a2), axis=1)
            r2 = self.reward_func(disc_input, discrim)
            if self.normalize_r:
                r2_mean, r2_std = torch.mean(r2), torch.std(r2)
                r2 = (r2-r2_mean)/r2_std

            # Target Q-values
            # Target policy smoothing
            # eps = torch.normal(torch.zeros(1), 0.01*torch.ones(1))
            q1_pi_targ = self.ac_targ.q1(o2, a2)
            q2_pi_targ = self.ac_targ.q2(o2, a2)
            q_pi_targ = torch.min(q1_pi_targ, q2_pi_targ)
            backup = self.gamma * (1 - d) * (r2 +   q_pi_targ - self.alpha * logp_a2)
    ...
    def compute_loss_pi(self,data,discrim):
        o = data['obs']
        pi, logp_pi = self.ac.pi(o[:, :self.true_state_dim])
        disc_input = torch.cat((o, pi), axis=1)
        for p in discrim.parameters():
            p.requires_grad = False
        r = self.reward_func(disc_input, discrim)
        ...
        if self.normalize_terms:
            r /= torch.std(r.detach())
            q_pi /= torch.std(q_pi.detach())
            # logp_pi /= torch.std(logp_pi.detach())

        # Entropy-regularized policy loss
        loss_pi_ent =  self.alpha * logp_pi.mean()
        loss_pi_r = -r.mean()
        loss_pi_q = -q_pi.mean()
        return loss_pi_ent, loss_pi_r, loss_pi_q, logp_pi

    def reward_func(self, data, discrim):
        disc_logits = discrim(data) # (B, 1)
        disc_logits = disc_logits.view(-1) # must squeeze
        # rewards = F.softplus(disc_logits, beta=-1) # -log(1-D) = -log(1+e^-D') ignore constant log2
        if self.objective in ['arc-f-max-rkl', 'naive-diff-f-max-rkl']:
            rewards = disc_logits
        elif self.objective in ['arc-gail', 'naive-diff-gail']:
            # rewards = torch.nn.functional.logsigmoid(disc_logits)
            rewards = torch.nn.functional.softplus(disc_logits, beta=-1)
            # not the same as gail definition!
        elif self.objective == 'arc-fairl':
            rewards = torch.exp(disc_logits)*(-1.0*disc_logits) # exp(D')*(-D')
        elif self.objective == 'arc-airl':
            reward = airl_reward_model(data).view(-1) # assuming no rew_shaping
        return rewards*self.reward_scale

    def comp_grad(self, prev = None):
        grad = []
        # print(self.ac.pi.parameters())

        with torch.no_grad():
            for p in self.ac.pi.parameters():
                grad += p.grad.cpu().numpy().flatten().tolist()
        
        grad = np.array(grad)
        grad = grad if prev is None else grad-prev
        return grad


    def update(self,data,discrim): # reward func must be differentiable with requires_grad=false
        # called by adv IRL
        ...
        # loss_pi_r.backward(retain_graph=True)
        # grad_1 = self.comp_grad()
        # loss_pi_q.backward(retain_graph=True)
        # grad_2 = self.comp_grad(grad_1)
        # loss_pi_ent.backward()
        # grad_3 = self.comp_grad(grad_2)
        loss_pi.backward()
        # print('Gradients', grad_1, grad_2, grad_3)
        # grad = (np.linalg.norm(grad_1), np.linalg.norm(grad_2), np.linalg.norm(grad_3))
        grad = (0,0,0)
        ...
        return np.array([loss_q.item(), loss_pi.item(), log_pi.detach().cpu().mean().item(), *grad])
