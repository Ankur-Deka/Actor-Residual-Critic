...
class Scenario(BaseScenario):
    def __init__(self, num_agents=1, dist_threshold=0.2, arena_size=1, identity_size=0, num_steps=100, diff_reward=True, same_color = False, random_leader_name=False, goal_at_top = False, hide_goal=False):
        ...
        self.start_loc = np.array([-0.7,-0.7], dtype = float)
    ...
    def make_world(self):
        ...
        x_coors = np.concatenate((np.linspace(-0.7,-0.7,50), np.linspace(-0.7,0.7,50))).reshape((-1,1))
        y_coors = np.concatenate((np.linspace(-0.7,0.7,50), np.linspace(0.7,0.7,50))).reshape((-1,1))
        world.traj_points = np.concatenate((x_coors, y_coors), axis=1)

        world.damping = 1
        world.agents[0].initial_mass = 0.03
        return world
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            ...
            agent.state.p_pos += np.random.uniform([-0.1, -0.1], [0.1,0.1])
    ...
    def reward(self, agent, world):
        ...
        # print(world.traj_points)
        # print(agent.state.p_pos)

        dists = np.linalg.norm(world.traj_points - agent.state.p_pos, axis=1)
        min_id = np.argmin(dists)

        # closeness to trajectory
        closeness_reward = -dists[min_id]

        # progress
        num_pts = world.traj_points.shape[0]
        progress_reward = min_id/(num_pts-1)

        reward = 10*closeness_reward + progress_reward
        return reward
    ...
    def observation(self, agent, world):
        ...
        default_obs = np.concatenate([agent.state.p_vel] + [agent.state.p_pos])
        # print(default_obs)
