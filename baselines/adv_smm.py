...
class AdvSMM:
    ...
    def __init__(
        self,
        env_fn, 
        obj, # f-max-rkl, gail, arc-gail, fairl, airl
        discriminator,
        agent, # e.g. SAC
        ...
        **kwargs   # for redundant arguments 
        ...
        print('mode ...')
        print(self.mode)
        assert self.mode in ['f-max-rkl', 'arc-f-max-rkl', 'gail', 'arc-gail', 'fairl', 'airl', 'naive-diff-gail', 'naive-diff-f-max-rkl'], 'Invalid adversarial irl algorithm!'
        if self.mode == 'airl':
        ...
        self.writer = SummaryWriter(self.logger.get_dir()+ '/tensorboard')
        ...
        for epoch in range(1, self.num_epochs+1):
            for steps_this_epoch in range(self.num_steps_per_epoch):
                ...
                if d or (ep_len==self.max_path_length):
                    # print(epoch, steps_this_epoch, ep_len)
                    o, ep_len = self.env.reset(), 0
    ...
    def _do_training(self, epoch):
        ...
        disc_total_loss = disc_stat[0]
        disc_ce_loss = disc_stat[1]

        self.writer.add_scalar('Discriminator/Total Loss', disc_total_loss, env_steps)
        self.writer.add_scalar('Discriminator/Cross Entropy Loss', disc_ce_loss, env_steps)

        # tensorboard - agent/policy
        policy_stat /= self.num_update_loops_per_train_call * self.num_policy_updates_per_loop_iter
        loss_q, loss_pi, log_pi, grad_r, grad_q, grad_entropy,_,_,_,_ = policy_stat
        self.writer.add_scalar('Agent/Q Loss', loss_q, env_steps)
        self.writer.add_scalar('Agent/Policy Loss', loss_pi, env_steps)
        self.writer.add_scalar('Agent/Policy Entropy', -log_pi, env_steps)
        
        self.writer.add_scalar('PolicyGradient/R', grad_r, env_steps)
        self.writer.add_scalar('PolicyGradient/Q', grad_q, env_steps)
        self.writer.add_scalar('PolicyGradient/Entropy', grad_entropy, env_steps)
    ...
    def process_target_state_buffer(self, target_state_buffer, expert_action_trajs):
        ...
        # else:
        print(len(target_state_buffer), len(expert_action_trajs))
    ...
    def get_target_batch(self, batch_size):
        if self.mode in {'gail', 'arc-gail', 'airl', 'arc-airl', 'f-max-rkl', 'arc-f-max-rkl', 'fairl', 'arc-fairl', 'naive-diff-gail', 'naive-diff-f-max-rkl'}:
            ...
            print(self.target_state_buffer.shape)
    ...
    def get_disc_input(self, batch):
        if type(batch) is dict:
            obs = batch['obs']
            act = batch['act']
        else:
            obs, act, _ = batch
        disc_input = torch.cat((obs, act), axis=1)
        return disc_input
    ...
    def _do_reward_training(self, epoch):
        ...
        if self.expert_IS: # assume we can only use policy states
            ...
            batch = self.replay_buffer.sample_batch(self.disc_optim_batch_size)
            policy_disc_input = self.get_disc_input(batch) 
            ...
            if self.mode in {'gail', 'arc-gail', 'f-max-rkl', 'arc-f-max-rkl', 'naive-diff-gail', 'naive-diff-f-max-rkl'}: # standard f-max
                ...
                batch = self.replay_buffer.sample_batch(self.disc_optim_batch_size)
                policy_disc_input = self.get_disc_input(batch) 
                # targets = self.bce_targets[self.disc_optim_batch_size:]
                # print(policy_disc_input.shape, 'SHAPE')
                # expert_batch = self.get_target_batch()
                # else:
                    # print('expert')
                expert_batch = self.get_target_batch(self.disc_optim_batch_size) # access to expert samples
                expert_disc_input = self.get_disc_input(expert_batch)
                # targets = self.bce_targets[:self.disc_optim_batch_size]
    ...
    def set_discriminator_requires_grad(self, val):
        for p in self.discriminator.model:
                p.requires_grad = val

    def _do_policy_training(self, epoch):
        policy_batch = self.replay_buffer.sample_batch(self.policy_optim_batch_size)
        
        if self.mode in ['arc-gail', 'arc-f-max-rkl', 'arc-fairl', 'naive-diff-gail', 'naive-diff-f-max-rkl']:
            # self.set_discriminator_requires_grad(False)
            # self.set_discriminator_requires_grad(True)
            obs, act = policy_batch['obs'], policy_batch['act']
            disc_input = torch.cat((obs, act), axis=1) ## doubt why not use policy actions for reward
            policy_batch['rew'] = self.get_reward(disc_input)

            agent_stat = self.agent.update(policy_batch, self.discriminator)
        
        elif self.mode in ['gail', 'f-max-rkl', 'airl', 'fairl']:
            # self.set_discriminator_requires_grad(True)
            obs, act = policy_batch['obs'], policy_batch['act']
            disc_input = torch.cat((obs, act), axis=1) ## doubt why not use policy actions for reward
            policy_batch['rew'] = self.get_reward(disc_input)
            agent_stat = self.agent.update(policy_batch)

        elif self.mode == 'arc-airl':
            agent_stat = self.agent.update(policy_batch, self.discriminator) # loss-q, loss-pi, log-pi. 
    ...
    def get_reward(self, obs, obs2=None):
        # if self.mode in ['arc-gail', 'naive-diff-gail']: # with torch_grad 
        #     with torch.no_grad():
        ...
        if not (self.mode in ['airl', 'arc-airl']):
            self.discriminator.eval() # changing mode to eval
            with torch.no_grad():
                disc_logits, _ = self.disc_forward(obs) # D' = log(D) - log(1-D)
                disc_logits = disc_logits.view(-1) # must squeeze
            self.discriminator.train()  # NOTE: changing back to train mode (NOT training!) 

            # NOTE: important: compute the reward using the algorithm
            if self.mode in ['f-max-rkl', 'arc-f-max-rkl', 'naive-diff-f-max-rkl']:
                rewards = disc_logits  # NOTE: log(D) - log(1-D) = D' = f(s,a)
            elif self.mode in ['gail', 'arc-gail', 'naive-diff-gail']:
                rewards = F.softplus(disc_logits, beta=-1) # -log(1-D) = -log(1+e^-D') ignore constant log2
            elif self.mode in ['fairl', 'arc-fairl']:
    ...
    def try_evaluate(self, policy_type: str, epoch):
        ...
        metrics = eval.KL_summary(expert_samples[0], agent_emp_states.reshape(-1, agent_emp_states.shape[2]), 
                                env_steps, policy_type, self.v['task']['task_name'] == 'uniform' if self.expert_IS else False) ## DOUBT in expert_samples[0]
        ...
        self.logger.dump_tabular()
        self.writer.add_scalar('Agent/Reward Deterministic', real_return_det, env_steps)
        self.writer.add_scalar('Agent/Reward Stochastic', real_return_sto, env_steps)
        if self.expert_IS:
